...
class Cell:
    """Объявляем класс - Органическая Клетка. Количество клеток может
    быть только целым числом"""

    def __init__(self, qty):
        if int(qty) == float(qty) and int(qty) > 0:
            self.qty = int(qty)
        else:
    ...
    def __sub__(self, other):
        return self.qty - other.qty if self.qty - other.qty > 0 \
            else print('Нельзя мЕньшую клетку уменьшать на бОльшую')

    def __floordiv__(self, other):
        return self.qty // other.qty if self.qty - other.qty > 0 \
            else print('Нельзя мЕньшую клетку делить на бОльшую')

    def __truediv__(self, other):
        return self.qty // other.qty if self.qty - other.qty > 0 \
            else print('Нельзя мЕньшую клетку делить на бОльшую')

    def make_order(self, ex):
        for i in range(self.qty // ex):
            print('*' * ex)
        print('*' * (self.qty % ex))
